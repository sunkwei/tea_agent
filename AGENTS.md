...
├── storage_scope.py                # 存储作用域：项目级 .tea_agent_run / 用户级 ~/.tea_agent
...
6. **存储作用域**：`TEA_STORAGE_SCOPE`（auto/project/user）决定 db 落在 `$pwd/.tea_agent_run/` 还是 `~/.tea_agent/`；新增持久化数据时须走 `storage_scope.resolve_db_path`，不要硬编码路径
...
| `TEA_STORAGE_SCOPE` | 存储作用域 | `auto`(默认)/`project`/`user` |
