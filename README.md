...
[![Version](https://img.shields.io/badge/version-0.13.11-blue)](https://pypi.org/project/tea-agent)
...
| 📋 **多对话管理** | 左侧会话列表面板，点击切换历史会话，自动加载消息。支持同时打开多个对话标签页，互不干扰 |
...
| 📎 **图片预览** | 消息中图片点击放大 |
| 🚦 **消息排队** | 当某个对话正在响应时，其他对话可排队等待，当前对话完成后自动发送排队消息。支持取消排队、队列状态可视 |
...
**🚦 消息排队机制** — 当某个对话（topic）正在响应时，用户可在其他对话中发送消息，该消息会自动进入该 topic 的队列。当前对话完成后，排队消息自动发送执行，无需等待。排队状态在 UI 上实时显示（队列图标 + 排队数量），用户也可随时取消排队中的消息。该机制确保多对话并发时的操作流畅性。
