...
| 模式 | 特点 | 适用场景 |
|------|------|----------|
| **lightweight** | 无存储、无后台线程 | 快速测试、脚本调用 |
| **full** | 完整存储 + 后台线程 | GUI、长期运行 |
| **lite** | 轻量级单轮、无历史 | 并行子任务、简单查询 |
...
# lite 模式（轻量级单轮）
agent = Agent(mode="lite", use_cheap_model=True)
```
...
当 API 返回 429 速率限制错误时，自动重试最多 3 次，递增等待时间：
...
### ④ 经验固化机制 (Experience Solidify)

任务完成后自动复盘，将经验转化为可复用技能：

```
任务完成 → 分析执行过程 → 成功？ → 固化到技能库 (dynamic_skill)
                        → 失败？ → 记录到经验库 (evolution_exp)
```
...
基于 jedi + ruff 的实时代码分析，支持 Python 和 C++：

| 功能 | Python (jedi) | C++ (clangd) |
|------|---------------|--------------|
| **诊断 (diagnose)** | ✅ ruff 静态检查 + jedi 语义错误 | ✅ clangd 诊断 |
| **补全 (completion)** | ✅ 上下文感知的代码补全 | ✅ ctags 符号补全 |
| **跳转定义** | ✅ 符号定义跳转 | ✅ ctags 定义跳转 |
...
# 自动检测语言并诊断
toolkit_lsp(action='diagnose', file='src/main.cpp')  # 自动使用 clangd
```
...
| `toolkit_format_code` | 代码格式化（Python black / C++ clang-format） |
...
| `toolkit_self_evolve` | 修改项目源码（五层安全：快照+备份+验证+LSP+测试） |
...
| `toolkit_experience_solidify` | 经验固化机制（自动复盘→技能/教训） |
| `toolkit_dynamic_skill` | 动态技能系统（记录成功模式） |
...
| `toolkit_export_last_pdf` | 导出会话为 PDF |

### 系统信息
| 工具 | 功能 |
|------|------|
| `toolkit_os_info` | 操作系统信息（进程缓存） |
| `toolkit_get_config_path` | 获取配置文件路径 |
| `toolkit_get_models` | 获取模型信息 |
...
| 工具 | 功能 |
|------|------|
| `toolkit_parallel_subtasks` | 分治并发执行器（lite agent 并行） |
| `toolkit_question` | 执行过程中向用户提问 |
| `toolkit_todo` | TODO 清单（DB 持久化） |
| `toolkit_stream_save` | STP 协议大文件写入 |
| `toolkit_write_b64` | Base64 编码大文件写入 |
| `toolkit_save_file` | 自动分块大文件写入 |
...
`toolkit_parallel_subtasks` 支持分治并发，将复杂问题分解为子任务，简单任务用 lite agent 并发执行：

```python
toolkit_parallel_subtasks(subtasks=[
    {"id": "task_1", "description": "分析文件 A", "difficulty": "easy"},
    {"id": "task_2", "description": "分析文件 B", "difficulty": "easy"},
    {"id": "task_3", "description": "复杂推理", "difficulty": "hard"},
])
# → task_1, task_2 用 lite agent 并发，task_3 由主 agent 执行
```
...
├── agent.py                    ← [统一 Agent 类](https://github.com/sunkwei/tea_agent/blob/master/tea_agent/agent.py) (lightweight/full/lite 三模式)
├── agent_core.py               ← [AgentCore 向后兼容](https://github.com/sunkwei/tea_agent/blob/master/tea_agent/agent_core.py)
...
├── litesession.py              ← [LiteSession](https://github.com/sunkwei/tea_agent/blob/master/tea_agent/litesession.py) (轻量级单轮)
│
├── session/                    ← [会话子包](https://github.com/sunkwei/tea_agent/blob/master/tea_agent/session/)
│   ├── _history_builder.py     ← 历史消息构建（L1/L2/L3 拼接）
│   ├── _json_sanitizer.py      ← JSON 校验与修复
│   ├── _os_info_injector.py    ← 操作系统信息注入
│   └── _tool_loop_runner.py    ← 工具调用循环执行器（含 429 重试）
│
...
│
├── toolkit/                    ← [60+ 个内置工具](https://github.com/sunkwei/tea_agent/tree/master/tea_agent/toolkit)
...
│   ├── toolkit_self_evolve.py  ← 五层安全自进化
│   ├── toolkit_experience_solidify.py ← 经验固化机制
│   ├── toolkit_dynamic_skill.py ← 动态技能系统
│   └── ... (50+ more)
│
├── _gui/                       ← [GUI 模块化子包 (23模块)]
│   ├── _tk_impl.py             ← Tk 实现层
...
│
├── lsp/                        ← [LSP 代码智能引擎]
│   ├── lsp_engine.py           ← jedi + ruff + clangd 诊断/补全/跳转
...
| 版本 | 关键变化 |
|------|---------|
| v0.9.13 | Agent 三模式架构（lightweight/full/lite）、C++ LSP 支持（clangd）、429 速率限制自动重试、Session 子包拆解、经验固化机制、动态技能系统、工具移动到内置工具箱（60+）、注释清理强化 pydoc |
| v0.9.12 | CNN 模型驱动的 Tetris AI 及训练数据生成工具 |
...
| v0.9.1 | `toolkit_js_fetch` Playwright 无头浏览器抓取（跨平台） |
