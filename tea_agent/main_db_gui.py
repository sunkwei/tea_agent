...
# NOTE: 2026-04-30 09:37:55, self-evolved by tea_agent --- 左侧主题列表移除token前缀，直接显示摘要标题（不超过20字）
    ...
    def switch_topic(self, topic_id):
        ...
        # 加载最近 N 条未摘要对话（分页，防止 GUI 卡顿）
        page_size = getattr(self, '_cfg', None)
        page_size = page_size.chat_page_size if page_size else 30
        conversations = self.db.get_unsummarized_conversations(topic_id, limit=page_size + 1)
        has_more = len(conversations) > page_size
        display_convs = conversations[:page_size]
        
        summary = self.db.get_topic_summary(topic_id) or ""
        self.sess.load_history(display_convs, summary)

        # 通知：有更早的对话被省略
        if has_more:
            self.log(f"📖 仅加载最近 {page_size} 轮对话，更早的已省略", "notice")
            self.log("")
        ...
        for c in display_convs:
            ...
            if c["is_func_calling"]:
                if tool_names:
                    self.log(f"ℹ️ 工具：{', '.join(tool_names)}", "tool")
                else:
