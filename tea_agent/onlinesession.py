...
# NOTE: 2026-05-17 08:50:28, self-evolved by tea_agent --- __init__ 中添加 _current_mode 追踪当前人格模式
        self.max_iterations = max_iterations
        self.enable_thinking = enable_thinking
        # @2026-05-17 gen by tea_agent, 追踪当前人格模式，用于模型参数差异化
...
# NOTE: 2026-05-07 17:31:36, self-evolved by tea_agent --- _process_stream_with_reasoning: reasoning_content 也回调给 GUI（[THINK]前缀标记）
    ...
    def _build_api_messages(self) -> List[Dict]:
        ...
        三级历史拼接：
            ...
            Level 1: 最新一轮压缩对话（含工具调用链，参数>2048B截断，输出首尾各1024B）
        """
        result: List[Dict] = []
    ...
    def _execute_tool_loop(self, context: Dict) -> Dict:
        ...
        if context.get("skip_tool_loop"):
            ...
            try:
                ...
                # 不传 tools，强制模型直接回答
# NOTE: 2026-05-17 08:51:24, self-evolved by tea_agent --- skip_tool_loop 也传入推理参数
# NOTE: 2026-05-17 08:52:02, self-evolved by tea_agent --- skip_tool_loop 使用 _get_effective_params helper
...
# NOTE: 2026-05-17 08:51:38, self-evolved by tea_agent --- 添加 _get_effective_params helper 统一获取推理参数
    # @2026-05-17 gen by tea_agent, 获取当前有效的模型推理参数（含模式覆盖）
    def _get_effective_params(self, model_type: str = "main") -> Dict[str, Any]:
        """返回 {temperature, max_tokens, top_p}，失败时返回空 dict（用 API 默认值）。"""
        try:
    ...
    def _auto_detect_mode(self, user_text: str):
        ...
        检测失败不阻塞对话。
        """
# NOTE: 2026-05-17 08:50:45, self-evolved by tea_agent --- _auto_detect_mode 更新 _current_mode 状态
        try:
            ...
            if isinstance(result, dict):
                ...
                # @2026-05-17 gen by tea_agent, 更新当前模式用于模型参数选择
