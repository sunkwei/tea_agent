...
def test_session_list_and_delete():
    ...
    # Step 9: session/delete
    input_sim.add({"jsonrpc":"2.0","id":9,"method":"session/delete","params":{"sessionId":sid}})
    time.sleep(0.15)
    r = _find(output_cap, 9)
    assert r and "result" in r
    print("  ✓ session/delete")

    # Step 10: providers/set
    input_sim.add({"jsonrpc":"2.0","id":10,"method":"providers/set","params":{"provider_id":"test","model":"fast"}})
    time.sleep(0.15)
    r = _find(output_cap, 10)
    assert r and "result" in r
    print("  ✓ providers/set")

    # Step 11: Unknown method
    input_sim.add({"jsonrpc":"2.0","id":11,"method":"unknown_method","params":{}})
    time.sleep(0.15)
