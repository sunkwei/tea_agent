...
# 路由表已抽到 server/_routes.py（同目录，故 Path(__file__).parent/"static" 语义不变）。
# noqa: E402 —— 必须置于上方 try/except(starlette) 之后：_routes 在模块级
# import starlette，若提前导入会让「未装 starlette」时的友好报错失效。
from ._routes import _build_routes  # noqa: E402
