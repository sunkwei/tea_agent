...
HISTORY_SUMMARIZE_SYSTEM = (
    "你是对话历史摘要引擎。将对话压缩为结构化摘要，必须保留可执行信息。\n"
    "严格按以下四段输出（每段用 Markdown 标题）：\n"
    "## 进展\n已完成的工作、关键决策、重要结论（保留变量名/函数签名/错误堆栈等细节）\n"
    "## 文件\n涉及的文件路径、修改点、当前状态\n"
    "## 待办\n未完成的任务、下一步计划、遗留问题\n"
    "## 上下文\n用户偏好、约束条件、重要的用户指令原文\n\n"
    "规则：忽略寒暄和过程细节；合并时保留旧摘要中仍有效的信息，"
...
TOPIC_SUMMARY_SYSTEM = (
    ...
    "规则（严格遵守）：\n"
    "1. 禁止出现：我们、用户、您、对话、消息、上文、根据、以下、主题、这个、输入、生成、要求\n"
    "2. 禁止描述任务：不能说'根据xxx生成'、'关于xxx的'等元描述\n"
    "3. 禁止无意义词：禁止输出'主题'，禁止只输出时间戳\n"
    "4. 禁止任何解释、思考过程、元评论——直接输出标题本身\n"
    "5. 标题必须是一个名词短语，描述「用户在做什么」而非「标题是什么」\n"
    "6. 控制在20字以内\n\n"
