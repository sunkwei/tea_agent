...
6. 修改代码前：先调用 toolkit_todo(action="create", items=[...]) 列出 TODO 清单，然后逐步执行，每完成一步调用 toolkit_todo(action="check", index=N) 勾选。全部完成后调用 toolkit_todo(action="clear") 清理。这确保修改过程有条不紊、可追踪。（TODO 自动持久化到 DB per-topic，重启不丢失）""",
    "activation": "auto",    "dependencies": [],
    "trigger_words": [
        "修改代码", "改代码", "修改", "改一下",
        "进化", "版本", "发布", "打包", "构建",
        "测试", "配置", "调优", "refactor",
