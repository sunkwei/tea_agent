...
class SkillCrystallizer:
    ...
    def _extract_steps(self, rounds: list[dict] | None, tools: list[str]) -> list[str]:
        """从对话轮次中提取执行步骤"""
        steps = []
    ...
    def _extract_intent(self, content: str) -> str | None:
        """从 AI 回复中提取意图"""
        # 简单启发式：找第一行描述性内容
    ...
    def _extract_success_conditions(self, rounds: list[dict] | None, task: str) -> list[str]:
        """提取成功条件"""
        conditions = []
    ...
    def _extract_tags(self, task: str, tools: list[str]) -> list[str]:
        ...
        # 从工具提取标签
        for tool in tools:
            if ("browser" in tool or "screenshot" in tool) and "浏览器" not in tags:
                tags.append("浏览器")
            if "test" in tool and "测试" not in tags:
                tags.append("测试")
