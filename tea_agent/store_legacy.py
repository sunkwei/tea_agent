...
# NOTE: 2026-05-15 15:09:47, self-evolved by tea_agent --- 创建 images 表用于存储图片二进制数据，修改 save_msg 实现图片持久化和 Base64 转换
    def _init_tables(self):
        ...
        ''')
        # NOTE: 2026-05-20 gen by tea_agent, 新增 images 表用于持久化存储聊天图片
        c.execute('''
...
# NOTE: 2026-05-15 15:10:13, self-evolved by tea_agent --- 修改 save_msg 方法，读取图片文件存入 images 表并转换为 Base64 嵌入 JSON
    # NOTE: 2026-05-20 gen by tea_agent, save_msg 支持图片持久化：读取文件存入 images 表并转为 Base64
    def save_msg(self, topic_id: str, user_msg, ai_msg: str, is_func: bool) -> int:
        '''
        新增一条，返回conversation_id。
        若 user_msg 含图片，自动读取文件存入 images 表，并将 images 字段替换为 Base64 数据以便直接显示。
        '''
        import json
        import base64
        c = self.conn.cursor()
            ...
            for img_item in raw_imgs:
                if os.path.isfile(img_item):
                    try:
                        ...
                        # 存入 images 表
                        c_img.execute('INSERT INTO images (conversation_id, image_blob, mime_type) VALUES (?, ?, ?)',
                                      (conv_id, blob, mime))
                        ...
                        import logging
                        logging.getLogger("store").error(f"Failed to process image {img_item}: {e}")
