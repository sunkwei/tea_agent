    ...
    def _chat_impl(self, user_input: str) -> List[Dict]:
        ...
        def status_cb(status_msg: str):
            ...
            if status_msg.startswith("!MAX_ITER:"):
                ...
                self._sess._extra_iterations += self._sess.context.extra_iterations_on_continue  # 2026-05-25 gen by Tea Agent, fix hardcoded续命轮数
                self._sess._max_iter_wait.set()
                self._notify({"type": "status", "text": f"已达最大轮次，自动续命{self._sess.context.extra_iterations_on_continue}轮..."})
            else:
