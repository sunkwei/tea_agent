...
def meta_toolkit_format_code() -> dict:
    return {"type": "function", "function": {"name": "toolkit_format_code", "description": "代码格式化工具。支持 Python (black) 和 C/C++ (clang-format) 格式化。 功能： - 格式化单个文件或目录 - 检查格式是否符合规范 - 自动检测语言并选择格式化工具 返回：格式化结果或错误信息", "parameters": {"type": "object", "properties": {"action": {"type": "string", "enum": ["format", "check"], "description": "format=格式化，check=检查"}, "path": {"type": "string", "description": "文件或目录路径"}, "language": {"type": "string", "enum": ["python", "cpp", "auto"], "description": "语言类型，默认 auto 自动检测"}, "style": {"type": "string", "description": "格式化风格，如 black 默认、Google、LLVM 等"}}, "required": ["action", "path"]}}}
