...
def meta_toolkit_plan():
    ...
    return {
        ...
        "function": {
            "name": "toolkit_plan",
            "description": "Plandex风格 Plan→Execute→Verify 任务工作流。create=创建计划, show=查看, step=执行单步, verify=验证步骤, run=执行全部, resume=从断点续跑, list=列出计划, delete=删除。计划持久化到 .tea_agent_run/plans/，支持依赖(depends_on)和验证(verify)。",
            "parameters": {
                "type": "object",
                "properties": {
                    "action": {"type": "string", "enum": ["create", "show", "step", "verify", "run", "resume", "list", "delete"], "description": "操作类型"},
                    "goal": {"type": "string", "description": "[create] 计划目标"},
                    "steps": {"type": "array", "items": {"type": "object"}, "description": "[create] 步骤列表 [{desc, action, depends_on, verify}]"},
                    "plan_id": {"type": "string", "description": "[show/step/run/resume/delete] 计划ID"},
                    "step_id": {"type": "string", "description": "[step/verify] 步骤ID"},
    ...
    try:
        ...
        if not action:
            if step.get("verify"):
                ...
                result = {"ok": True, "info": "checkpoint"}
