...
def meta_toolkit_stream_save() -> dict:
    return {"type": "function", "function": {"name": "toolkit_stream_save", "description": "STP流缓冲区落盘。将模型通过流协议输出的内容写入文件，绕过JSON工具调用参数大小限制。", "parameters": {"type": "object", "properties": {"stream_id": {"type": "string", "description": "The stream key from ] marker"}, "target_path": {"type": "string", "description": "Override target file path (optional, uses path from stream header if not set)"}, "append": {"type": "boolean", "description": "Append to file instead of overwriting"}}, "required": ["stream_id"]}}}
